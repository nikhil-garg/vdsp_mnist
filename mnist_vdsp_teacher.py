...
def vdsp(w, vmem, lr=1, lr_pn=1):
    ...
    dW = cond_pot * f_pot * g_pot * lr - cond_dep * f_dep * g_dep * lr * lr_pn
    return dW
...
def run_one_sample(
    ...
    for t in range(duration_per_sample):
        ...
        if np.any(output_spikes):
            ...
            if use_vdsp:
                vapp = mem_pot_input + vdsp_offset
                dw = vdsp(weights[:, spiking_neuron], vapp, vdsp_lr, lr_pn)  # For its connection, get dws
                ...
                # with additive stdp, normalization is required
                weights = normalize_weights(weights, norm_scale)
            mem_pot_output[spiking_neuron] = output_reset  # reset mem pot
            refractory_neurons[spiking_neuron] = refractory_period_output  # Set refrac
...
def main(
    ...
    input_reset=-1,
    input_bias=0.032,
    refractory_period_input=5,
    refractory_period_output=5,
...
def run_random_vdsp_teacher(seed):
    ...
    accuracy = run_vdsp(
        ...
        input_leak_cst=np.exp(-1 / 30),
        output_leak_cst=np.exp(-1 / 60),
        input_threshold=np.random.uniform(0.2,2),
        output_threshold=np.random.uniform(1,100),
        input_reset=np.random.uniform(-0.2,-2),
        output_reset=-1,
        input_bias=0,
        refractory_period_input=int(np.random.uniform(1,20)),
        refractory_period_output=int(np.random.uniform(1,20)),
        lateral_inhibition_period=int(np.random.uniform(5,50)),
        input_scale=np.random.uniform(0.001,0.01),
        nb_epochs=1,
        use_vdsp=True,
        vdsp_lr=0.001,
        lr_pn=np.random.uniform(0.25,4),
        vdsp_offset=0,
        teacher_stimuli_strength=np.random.uniform(0.01,4),
        tau_pre=150,
        tau_post=150,
        dapre=0.00000001,
        dapost=0.0001,
...
def run_random_vdsp(seed):
    ...
    accuracy = run_vdsp(
        ...
        lr_pn=np.random.uniform(1/2, 2*1),
        with_validation=False,
        th_leak_cst=np.exp(-1 / 1000),
        th_inc=0.0,  # 0 is no adaptive threshold
...
def run_random_stdp(seed):
    ...
    accuracy = main(
        ...
        output_threshold=1,
        input_bias=0,
        refractory_period_input=np.random.randint(1, 5),
        lateral_inhibition_period=np.random.randint(1, 50),
